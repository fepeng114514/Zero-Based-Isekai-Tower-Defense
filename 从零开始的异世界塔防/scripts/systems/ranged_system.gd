...
func on_update(delta: float) -> void:
	for e in EntityDB.entities:
		if not Utils.is_vaild_entity(e) or not e.has_c(CS.CN_RANGED):
			continue
		...
		for a: Dictionary in ranged_c.order:
			if not state & CS.STATE_IDLE and (not a.together_melee or state & CS.STATE_MELEE):
				continue
...
func attack(e: Entity, a: Dictionary) -> void:
	var target = EntityDB.search_target(a.search_mode, e.position, a.min_range, a.max_range, a.flags, a.bans)
	if not target:
		return
