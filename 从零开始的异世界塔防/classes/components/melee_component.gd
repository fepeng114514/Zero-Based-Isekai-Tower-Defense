...
@export ## 是否是拦截者，表示实体是否具有拦截能力
var is_blocker: bool = false
## 被拦截者 ID 列表，表示实体当前正在拦截的被拦截者 ID 列表
var blockeds_ids: Array[int] = []
@export ## 是否是被动障碍，表示实体是否作为被动障碍存在（不主动拦截敌人），通常用于某些特殊的实体，例如路障等
var is_passive_obstacle: bool = false
@export ## 拦截最小范围，单位为像素
var block_min_range: float = 80
@export ## 拦截最大范围，单位为像素
var block_max_range: float = 0
@export ## 搜索模式，表示实体在寻找被拦截者时的目标选择策略，默认为优先第一个敌人
var search_mode: C.SEARCH = C.SEARCH.ENEMY_FIRST
@export ## 最大可以被拦截数量，表示实体最多可以同时拦截多少个被拦截者，超过该数量后将不再拦截新的被拦截者
